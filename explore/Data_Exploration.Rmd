...
con <- dbConnect(drv, dbname="w205project", host="ec2-34-228-75-222.compute-1.amazonaws.com", 
...
ggplot(df_local_revenue_range_score, aes(x = local_revenue_per_student, y = math_score, colour = state)) + geom_point() + 
  geom_line() + labs(x = "Local Revenue per Student", y = "Math Score", title = "Math Score by Total Revenue per Student")
...
Our exploratory analysis found a possible relationship between revenue per student and math or reading scores. 

We also found that there may be a closer relationship between local revenue and academic outcome than other sources of revenue.

When we examined where the funding was being spent we did see a possible relationship between smaller class sizes and educational outcomes.

Taking this research further in a future project, it would be interesting to look in more depth at the way the funding is spent. Our initial analysis has focused on a subset of the Common Core of Data, so there is scope to expand this investigation.
